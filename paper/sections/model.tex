...
These assumptions yield five attack surfaces: \emph{omission and replacement}
by a coordinator; \emph{strategic observation} before the election closes;
\emph{local retaliation} after publication; malformed encrypted submissions
that hide negative, oversized, or wrong-sum tallies; and opening
inconsistencies in the trustee registry or decryption transcript.

The protocol answers these surfaces with complete-set finalization, transcript
inclusion, and encrypted submissions: local tallies are never revealed, only
the final aggregate is opened, malformed submissions are excluded through
zero-knowledge proofs, and trustee setup and opening are proof-checked.
